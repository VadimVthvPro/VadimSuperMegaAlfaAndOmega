...
def choise_of_mass(message):
    ...
    cursor.execute(f"UPDATE users SET user_weight = ?, imt = ?, imt_str = ? WHERE user_id = ? AND date = ?",
                   (weight, imt, imt_using_words, message.from_user.id, datetime.datetime.now().strftime('%Y-%m-%d')))
    conn.commit()
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    btn1 = 'Сброс веса'
...
def aim(message):
    ...
    conn.commit()
    cursor.execute(
        "SELECT user_height, user_weight,imt, imt_str FROM users WHERE user_id = {}".format(int(message.from_user.id)))
