...
async def counting_users_cal_after_train(user_id: int, date: str, user_train_cal: float, tren_time: int):
    async with aiosqlite.connect('pro3.db') as conn:
        await conn.execute(
...
async def tren_len(message: Message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, text=f'Прекрасно! Ты за тренировку сжёг {tren_cal} килокалорий. Так держать!!',
                     reply_markup=kb.main_menu)
    await counting_users_cal_after_train(user_id=message.from_user.id, date=datetime.datetime.now().strftime('%Y-%m-%d'),
                                   user_train_cal=tren_cal, tren_time=time)
    async with aiosqlite.connect('pro3.db') as conn:
        await conn.execute(
    "SELECT SUM(user_train_cal) FROM user_training_cal WHERE date = ? AND user_id = ?",
                   (datetime.datetime.now().strftime('%Y-%m-%d'), message.from_user.id))
    col_cal_tren = cursor.fetchone()[0]
