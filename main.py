...
bot = telebot.TeleBot('7239777184:AAEJCJcJx4_c9fuj9pIFKT3RlofrcnVV9Nk')
...
за день было употреблено Б/Ж/У в соотношении {col_b if col_b else 0}/{col_g if col_g else 0}/{col_u if col_u else 0} и выпито {col_wat * 300 if col_wat else 0} милилитров воды. Я верю в твой успех!!""", reply_markup=alfamarkup)
    ...
    if weight_month and sr_b and sr_g and sr_u and sr_cal and sr_tren and sr_w and weight_data:
...
def start(message):
  ...
  markup.add(btn1, btn2)
  ans = bot.send_photo(message.chat.id, photo = p, caption = 'Привет, {}! Бот PROпиташка поможет тебе вести индивидуальный расчет твоего питания и активности, опираясь на твои персональные параметры)'.format(message.from_user.first_name), reply_markup=markup)
